...
def upload_dalle_image_to_s3(image_url):
    """Downloads image from DALL-E URL and uploads to S3"""
    try:
        # Download image
        response = requests.get(image_url)
        if response.status_code != 200:
            return {"errors": f"Failed to download image: {response.status_code}"}

        # Generate filename
        filename = f"dreamscape_{datetime.now().strftime('%Y%m%d_%H%M%S')}.png"
